...
exports.findById = function (id) {
  for (var i = 0; i < trips.length; i++) {
    if (trips[i].eid == id) return trips[i];
  }
};

exports.findByTag = function (tag) {
  return trips.filter((title) => {
    return (
      //filter title
      title.title.match(tag) ||
       //filter Description
      title.description.match(tag) ||
       //filter tags for index array
      title.tags[0].match(tag)
    );
  });
};
